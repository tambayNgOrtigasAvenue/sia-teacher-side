...
if (
    ...
    // Prepare SQL query
    $query = "UPDATE announcement SET author_id = ?, title = ?, content = ?, publish_date = ?, expiry_date = ?, target_audience = ?, is_pinned = ?, is_active = ? WHERE id = ?";

    $stmt = $db->prepare($query);

    // Execute query
    if ($stmt->execute([$author_id, $title, $content, $publish_date, $expiry_date, $target_audience, $is_pinned, $is_active, $announcement_id])) {
        // Set response code - 200 OK
        http_response_code(200);

        // Tell the user
        echo json_encode(array("message" => "Announcement was updated."));
    } else {
        // Set response code - 503 service unavailable
        http_response_code(503);

        // Tell the user
        echo json_encode(array("message" => "Unable to update announcement."));
    }
} else {
    ...
    // Tell the user
    echo json_encode(array("message" => "Unable to update announcement. Data is incomplete."));
}