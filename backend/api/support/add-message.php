...
try {
    ...
    if (empty($data['ticketId']) || empty($data['message'])) {
    ...
    $result = $controller->addTicketMessage(
        $data['ticketId'],
        $userId,
        $data['message']
    );
    ...
    } else if (isset($result['error_code']) && $result['error_code'] === 'TICKET_CLOSED') {
        http_response_code(400);
    } else {
        http_response_code(400);
    }
    ...
    error_log("Error in add-message.php: " . $e->getMessage());
